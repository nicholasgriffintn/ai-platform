...
export interface ComposioToolkitAuthConfig {
  id: string;
  name: string;
  authScheme: string;
  isManaged: boolean;
...
export interface ComposioToolkitOperation {
  id: string;
  access: "read" | "write";
  readOnly: boolean;
  destructive: boolean;
...
export type ConfiguredComposioToolkit = {
  providerId: string;
  name: string;
  description: string;
  logoUrl: string;
  ...
  authConfigs: ComposioToolkitAuthConfig[];
  toolkitSlug: string;
  ...
  scopes: string[];
  operations: ComposioToolkitOperation[];
};
...
const toolkitSchema = z
  ...
  .transform((toolkit): ConfiguredComposioToolkit => {
    ...
    const toOperation = (id: string, access: "read" | "write"): ComposioToolkitOperation => ({
      id,
      access,
...
export const composioToolkitCatalogueSchema = z.object({
  toolkits: z.record(z.string(), toolkitSchema),
});

export type UnparsedComposioToolkitCatalogue = z.input<typeof composioToolkitCatalogueSchema>;
export type ComposioToolkitCatalogue = z.infer<typeof composioToolkitCatalogueSchema>;
