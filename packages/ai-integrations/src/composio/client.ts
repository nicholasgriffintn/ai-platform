...
import type { ComposioAuthConfigDefinition, ConnectorProviderConfig } from "../providers.js";
...
function requireUserNamespace(env: ComposioEnvironment): string {
...
export function getComposioUserId(env: ComposioEnvironment, userId: number): string {
  return `polychat:${requireUserNamespace(env)}:user:${userId}`;
}

export function isComposioConfigured(env: ComposioEnvironment): boolean {
  try {
...
export async function createComposioToolSession(params: {
  ...
  const connectedAccountAuthConfigId = params.connectedAccount.authConfigId;
  const availableTools = new Set(
    params.provider.operations
      .filter((operation) => operation.authConfigIds?.includes(connectedAccountAuthConfigId))
