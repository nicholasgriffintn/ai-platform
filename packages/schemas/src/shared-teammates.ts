...
export const sharedTeammateSummarySchema = z.object({
  id: z.string(),
  name: z.string(),
...
export const rateTeammateSchema = z.object({
  rating: z.int().min(1).max(5).meta({ description: "Rating from 1 to 5 stars" }),
  review: z.string().optional().meta({ description: "Optional review text" }),
});

export const sharedTeammateFiltersSchema = z.object({
  ...
  offset: z.int().min(0).prefault(0).optional().meta({ description: "Number of results to skip" }),
  sort_by: z
...
export const featuredTeammatesSchema = z.object({
  limit: z.coerce
    .number()
    .min(1)
    .max(50)
