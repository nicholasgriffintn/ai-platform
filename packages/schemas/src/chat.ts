...
export const createChatCompletionsJsonSchema = z.object({
	completion_id: z.string().optional().meta({
		description: "The ID of the completion to be used for future requests if stored.",
	}),
	...
	should_think: z.boolean().optional().meta({
		description: "Whether to enable thinking mode for the model. (Used for Claude Sonnet 3.7).",
	}),
	use_multi_model: z
		.boolean()
		.optional()
		.describe("Whether to use multiple models to generate the response when appropriate"),
	messages: z
		.array(
			z.object({
				role: z.enum(["developer", "system", "user", "assistant", "tool"]).meta({
							...
							z
								.object({
									...
									image_url: z
										.object({
											...
											detail: z.enum(["auto", "low", "high"]).optional().prefault("auto").meta({
								...
								.refine(
									(obj) => {
										...
										if (obj.type === "markdown_document") return !!obj.markdown_document;
										return true;
									},
									{
										path: ["type"],
				...
				parts: messagePartsSchema.optional().meta({
					description: "Structured message parts including text, reasoning, tools, and files.",
				}),
	...
	top_k: z.number().min(1).max(100).optional().meta({
		description: "The number of top most likely tokens to consider for the model to sample from.",
	}),
	...
	tools: z
		...
		.optional()
		.meta({
			description: "A list of tools that the model can use to respond to the user's message.",
		}),
	tool_choice: z
		.union([
			z.literal("none"),
	...
	budget_constraint: z.number().optional().meta({
		description: "The maximum amount of money the user is willing to spend on the completion.",
	}),
