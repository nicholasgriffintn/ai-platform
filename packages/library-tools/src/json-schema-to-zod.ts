...
function parsePattern(pattern: string): RegExp {
  try {
    ...
    throw new ToolError(
      "invalid_schema",
      `Invalid tool parameter regex pattern: ${error instanceof Error ? error.message : String(error)}`,
      { pattern },
    );
  }
}
...
function applyStringRules(schema: z.ZodString, property: JsonSchemaProperty): z.ZodTypeAny {
  ...
  if (typeof property.minLength === "number") {
    next = (next as z.ZodString).min(property.minLength);
  }

  if (typeof property.maxLength === "number") {
    next = (next as z.ZodString).max(property.maxLength);
  }

  if (property.pattern) {
    next = (next as z.ZodString).regex(parsePattern(property.pattern));
  }
