...
export type PlatformEnvKeyGroups = string[][];

function hasEnvValue(env: PlatformEnv, key: string): boolean {
  const value = env[key];

  return typeof value === "string" && value.trim().length > 0;
}

const EMPTY_GROUPS: PlatformEnvKeyGroups = [];

function single(key: string): PlatformEnvKeyGroups {
  return [[key]];
}

export const PROVIDER_PLATFORM_ENV_KEYS: Record<string, PlatformEnvKeyGroups> = {
  ...
  typesafe: single("TYPESAFE_API_KEY"),
  "typesafe-ai": single("TYPESAFE_API_KEY"),
...
export function getProviderPlatformEnvKeys(providerId: string): PlatformEnvKeyGroups | undefined {
...
export function getPlatformEnabledProviders(env: PlatformEnv): Set<string> {
