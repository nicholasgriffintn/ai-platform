...
import { getToolDefinitionName } from "@ngriffin_uk/polychat-utility-server/tool-names";

import { formatToolCalls, getToolDefinitionLikeName } from "../tool-definitions.js";
...
class OpenAIResponsesToolBuilder {
  ...
  private buildToolSearchNamespaces(): any[] {
    ...
    const deferredFunctions = (this.params.deferred_functions ?? []).filter(
      (tool) => !immediateNames.has(getToolDefinitionLikeName(tool)),
    );
