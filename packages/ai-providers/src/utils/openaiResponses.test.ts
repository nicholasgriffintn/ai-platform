...
import { buildOpenAIResponsesBody, shouldUseOpenAIResponsesApi } from "./openaiResponses.js";
...
describe("shouldUseOpenAIResponsesApi", () => {
  it("forces the responses API for models that only support it", () => {
    for (const modelId of [
      ...
      const modelConfig = catalogueModel("openai", modelId);
  ...
  it("keeps chat completions for models that do not require the responses API", () => {
    const modelConfig = catalogueModel("openai", "gpt-5.2");
  ...
  it("uses the responses API when a message contains a document", () => {
    const modelConfig = catalogueModel("openai", "gpt-5.6");
  ...
  it("uses the responses API for hosted tools", () => {
    const modelConfig = catalogueModel("openai", "gpt-5.4");
  ...
  it("does not force the responses API for non-text output models", () => {
    const imageModel = catalogueModel("openai", "gpt-image-2");
...
describe("current OpenAI model capabilities", () => {
  it("matches the reasoning defaults in the current model guidance", () => {
    expect(catalogueModel("openai", "gpt-4.1").reasoningConfig).toBeUndefined();
    expect(catalogueModel("openai", "gpt-5").reasoningConfig).toMatchObject({
      supportedEffortLevels: ["minimal", "low", "medium", "high"],
      defaultEffort: "medium",
    });
    expect(catalogueModel("openai", "gpt-5.1").reasoningConfig?.defaultEffort).toBe("none");
    expect(catalogueModel("openai", "gpt-5.2").reasoningConfig?.defaultEffort).toBe("none");
    expect(
      catalogueModel("openai", "gpt-5.3-codex").reasoningConfig?.supportedEffortLevels,
    ).toEqual(["low", "medium", "high", "xhigh"]);
    expect(catalogueModel("openai", "gpt-5.4").reasoningConfig?.defaultEffort).toBe("none");
    expect(catalogueModel("openai", "gpt-5.5").reasoningConfig?.defaultEffort).toBe("medium");
    expect(catalogueModel("openai", "gpt-5.6").reasoningConfig?.defaultEffort).toBe("medium");
    expect(catalogueModel("openai", "gpt-6-astra").reasoningConfig).toMatchObject({
  ...
  it("exposes Fast processing only on supported OpenAI models", () => {
    for (const modelId of [
      ...
      expect(catalogueModel("openai", modelId).supportedServiceTiers, modelId).toEqual([
        "default",
        "fast",
      ]);
    }

    expect(catalogueModel("openai", "gpt-5.5").supportedServiceTiers).toBeUndefined();
  });

  it("forwards the selected service tier to the Responses API", () => {
    const body = buildOpenAIResponsesBody(
      { ...baseParams, service_tier: "fast" },
      catalogueModel("openai", "gpt-6-astra"),
    );
  ...
  it("keeps pro-model hosted tool restrictions accurate", () => {
    expect(catalogueModel("openai", "gpt-5.4-pro")).toMatchObject({
      supportsCodeExecution: false,
      supportsComputerUse: true,
      supportsToolSearch: true,
    });
    expect(catalogueModel("openai", "gpt-5.4-pro").supportsHostedShell).toBeUndefined();
    expect(catalogueModel("openai", "gpt-5.5-pro")).toMatchObject({
      supportsCodeExecution: true,
      supportsHostedShell: true,
      supportsStreaming: false,
    });
    expect(catalogueModel("openai", "gpt-5.5-pro").supportsComputerUse).toBeUndefined();
    expect(catalogueModel("openai", "gpt-5.5-pro").supportsToolSearch).toBeUndefined();
  });
  ...
  it("requests Luna reasoning summaries and code interpreter outputs by default", () => {
    const body = buildOpenAIResponsesBody(
      ...
      catalogueModel("openai", "gpt-5.6-luna"),
    );
