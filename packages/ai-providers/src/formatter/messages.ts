...
import { estimateMessageTokens } from "../message-tokens.js";
import type { ContentType, Message, MessageContent } from "../types/index.js";
...
export class MessageFormatter {
        ...
        if (provider === "anthropic") {
          const toolCallArguments =
            typeof message.tool_call_arguments === "string"
              ? (safeParseJson<Record<string, unknown>>(message.tool_call_arguments) ?? undefined)
  ...
  private static takeMessagesAroundMiddle(messages: Message[], maxTokens: number): Message[] {
    ...
    const addIfFits = (index: number): boolean => {
      ...
      if (!message || selectedIndices.has(index)) {
        return false;
      }
  ...
  private static getBase64FromUrl(dataUrl: string): string {
    return dataUrl.match(/^data:([^;]+);base64,(.+)$/)?.[2] ?? dataUrl;
  }
}
