...
import { buildOpenAIResponseOutputParts } from "../openai-response-parts.js";
import { preprocessQwQResponse } from "../unterminated-thinking.js";
...
export class ResponseFormatter {
  ...
  private static filterUrlsByExtension(urls: string[], extensions: string[]): string[] {
    return urls.filter((url) => {
      const normalized = (url.split("?")[0] ?? url).toLowerCase();
  ...
  private static async formatOpenAIResponse(
    ...
    if (isImageType && Array.isArray(data.data)) {
      const imageData: Array<{ url?: string; b64_json?: string; revised_prompt?: string }> =
        Array.isArray(data.data) ? data.data.filter(isRecord) : [];
