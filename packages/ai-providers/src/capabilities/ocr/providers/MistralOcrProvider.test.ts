...
import { createTestRuntime, createTestStorage } from "../../../__test__/test-runtime.js";
import type { ProviderEnv, ProviderUser } from "../../../env.js";
import { createCatalogueModelResolver } from "../../../model-resolver.js";
import { MistralOcrProvider } from "./MistralOcrProvider.js";
...
vi.mock("../../../credentials.js", () => ({
  hasHostUserProviderApiKey: vi.fn(),
  resolveHostProviderApiKey: mocks.resolveProviderApiKey,
}));

const runtime = createTestRuntime({
  models: { ...createCatalogueModelResolver(), resolveModelConfig: mocks.resolveModelConfig },
...
describe("MistralOcrProvider", () => {
  ...
  it("sends the current OCR 4 request contract without local storage metadata", async () => {
    await new MistralOcrProvider(runtime).extractText({
      env,
      user,
      id: "request-1",
      document: {
  ...
  it.each([
    ...
    await new MistralOcrProvider(runtime).extractText({ env, user, document });
  ...
  it("returns bounded text and the normalised OCR structure", async () => {
    ...
    const result = await new MistralOcrProvider(runtime).extractText({
      env,
      user,
      document: {
        type: "document_url",
