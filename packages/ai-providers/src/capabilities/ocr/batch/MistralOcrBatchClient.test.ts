...
import { MistralOcrBatchClient } from "./MistralOcrBatchClient.js";

const runtime = createTestRuntime();

vi.mock("../../../credentials.js", () => ({
  resolveHostProviderApiKey: vi.fn().mockResolvedValue("mistral-key"),
}));
...
const user = { id: 42 } as ProviderUser;
...
describe("MistralOcrBatchClient", () => {
  ...
  it("starts an OCR batch with bounded inline requests", async () => {
    ...
    const result = await new MistralOcrBatchClient(runtime).start({
      env,
      user,
  ...
  it("rejects a successful response that is not a complete batch job", async () => {
    ...
    await expect(
      new MistralOcrBatchClient(runtime).get({ env, user, jobId: "job-1" }),
