...
export class OpenAIProvider extends BaseProvider {
  ...
  private isImageGeneration(params: ChatCompletionParameters): boolean {
    return params.model?.startsWith("gpt-image-") ?? false;
  }
  ...
  private handleImageToImageRequest(
    ...
    const imageMessage = params.messages?.[1];
    ...
    const imageUrls = imageMessage.content
  ...
  async mapParameters(
    ...
    if (isImageEditing || isTextToImage) {
      ...
      const promptMessage = messages.length > 1 ? messages[1] : messages[0];
      const prompt =
      ...
      if (endpoint.includes("images/edits") && hasImages) {
        if (!_storageService) {
          throw new AssistantError(
            "ProviderStorage is required for image editing",
            ErrorType.CONFIGURATION_ERROR,
          );
        }
