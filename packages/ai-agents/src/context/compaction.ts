...
import { hasCompactionPart, isCompactionMarkerMessage } from "@ngriffin_uk/polychat-schemas";
...
function countsTowardCompactionPressure(message: CompactionPlanMessage): boolean {
  return !isCompactionMarkerMessage(message) && !hasCompactionPart(message.parts);
}
