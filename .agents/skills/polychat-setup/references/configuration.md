...
- **Task notifications:** set the existing `PRIVATE_KEY` for authenticated encryption, a public `WEB_PUSH_PUBLIC_KEY`, and an HTTPS `TASK_NOTIFICATION_PROVIDER_URL` with its secret `TASK_NOTIFICATION_PROVIDER_TOKEN`. The gateway owns Web Push VAPID private material and any APNs signing credentials, and must deduplicate the `Idempotency-Key` header before invoking a provider; read the delivery service for the current envelope fields. Browser delivery additionally requires a secure origin and the shipped service worker.
- **iOS Work notifications:** these still go from the API to APNs directly and need `APNS_KEY_ID`, `APNS_TEAM_ID`, `APNS_PRIVATE_KEY` and `APNS_TOPIC`, plus the Apple Push Notifications capability and matching provisioning profile for the iOS bundle. Leave either notification path unavailable when its credentials are absent; operating-system permission alone is not a successful registration. Live Activities are not configured.
