...
Credit admission reads persisted plan allowances; missing allowances refuse work. Stored run usage groups ledger events by stable run and attempt but keeps the reservation, recorded consumption and settlement distinct, and missing provider telemetry is unknown rather than zero. Expired-reservation cleanup fences release with the scanned reservation ID so an older cleanup cannot release a replacement hold. Workspace reporting is attribution, not a shared balance. See [usage operations](../operations/loop-cost-controls.md) and [0022](decisions/0022-meter-vendor-units-and-settle-once.md).
...
`apps/app/vite.config.ts` resolves browser-only packages — the WebLLM engine, `@babel/standalone`, Strudel and `posthog-js` — to `workers/browser-only-stub.ts` in the SSR environment alone, so the Cloudflare Worker carries no code that can only run after hydration. Add a package to that list when it is reachable solely from a browser effect; the client build keeps the real module behind its existing dynamic import.
...
`packages/component-ui` owns every form control. `Input`, `Textarea`, `Checkbox`, `Switch` and `RangeInput` are the only components that render a raw form element; the `Form/*` wrappers compose them and add the label, description and identifiers. `FormSelect` is the single dropdown for the whole product, backed by `DropdownMenu` rather than a native `<select>`, generic over its option value, and driven by `onValueChange`. Feature packages compose these components and add neither their own primitives nor a second dropdown. See [0043](decisions/0043-one-form-control-set-and-one-dropdown.md).
...
Image Studio uses project scope throughout Canvas and Drawing reads and generation requests. Authorise project app access before invoking a provider, and preserve the project on generated Outputs and original Sources. Drawing group IDs associate results; fresh storage IDs keep repeated uploads from overwriting earlier files.
