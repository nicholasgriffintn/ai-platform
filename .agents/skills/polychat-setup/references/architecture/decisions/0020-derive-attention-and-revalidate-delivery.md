...
Inbound automated channel messages use a separate calibrated triage policy for `needs_reply` and `needs_attention`. Treat the signals independently: a message may be replied to, replied to and marked for attention, recorded for attention without an external reply, or confidently ignored. Uncertainty preserves the reply path. Revalidate the binding after judgement. Attention-only events add an idempotent bounded notification to the authoritative channel conversation and mark it unread; replied attention events mark that same conversation unread after delivery. Neither creates an Attention record or copies channel prose into Work Attention, so the read model in this decision remains task- and run-derived.
