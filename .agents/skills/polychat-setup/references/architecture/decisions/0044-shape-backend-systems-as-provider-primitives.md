...
- `ai-agents` owns context management (context budgets, compaction plans, window limits) and builds `Agent` on `library-agent-loop` (the decision loop, renamed from `agent-core`), `library-tools` (definitions, catalogues, validation, execution, replacing `library-tool-runtime` and the tool registry schema) and `library-workflows` (workflows, events, schedules, workers).
- `library-tasks` holds outcome settlement, execution leases, handler registries and status machines. The API's task executors and project task transitions use it through a `LeaseStore` adapter.
