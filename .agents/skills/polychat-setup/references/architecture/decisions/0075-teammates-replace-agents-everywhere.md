...
Renaming four tables and five columns is a data migration, and `drizzle-kit generate` resolves renames through an interactive prompt that needs a TTY, which CI and agent sessions do not have. Rather than leave storage on the old names, the rename ships as an authored `ALTER TABLE ... RENAME` migration paired with a snapshot generated from the schema by `scripts/generate-rename-snapshot.mts`, which calls drizzle's own `generateSQLiteDrizzleJson`. The snapshot is generated, not hand-edited, so `db:generate` reports no pending diff afterwards. Use that script for any future rename; every other change still goes through `db:generate` unchanged.

Renaming the capability kind updates existing project rows in the same migration. Anything that stored the literal `agent` outside those tables, such as an exported project template written before this change, keeps the old value and will not match.
