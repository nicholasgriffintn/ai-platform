...
A **theme** is a named palette with a declared appearance. `packages/component-ui/src/theme.ts` is the registry: it owns the identifiers, labels, appearance and storage keys, and it derives the pre-paint bootstrap script from the same data. A **theme preference** is what the person chose and may be `system`; a **theme id** is what that resolves to against `prefers-color-scheme`. A **theme pair** names the light theme and the dark theme that `system` resolves between; it is stored separately from the preference as `light:dark`, defaults to Light and Dark, and is rejected as a whole when either half has the wrong appearance, so a stale or hand-edited value falls back rather than producing a light palette at night.
...
Every palette must be authored and checked against contrast and sRGB gamut for each theme rather than inherited from Tailwind's ramps, and a new theme needs that work before it can ship. Adding an identity hue means choosing a chroma that survives every theme's lightness. The pair is a second stored value that the bootstrap script and the store must both read the same way, which the parity test enforces but which is one more thing to keep aligned. In return one attribute drives the whole interface, previews are real rather than illustrative, and four further palettes needed no changes to feature code.
