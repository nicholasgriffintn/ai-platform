...
Publish `supportedServiceTiers` separately from `serviceTierMultipliers`. The former controls which explicit processing modes clients may offer; the latter prices the tier reported by the provider. Automatic processing is represented by omitting `service_tier`, while Standard sends `default` and Fast sends `fast`. OpenAI may downgrade Fast requests to standard processing, so meter the response tier rather than the requested tier.
