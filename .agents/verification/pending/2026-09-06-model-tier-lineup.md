...
- **Prerequisites:** D1 migrations `0028_add_project_default_model_tier`, `0029_map_project_default_model_tier` and `0030_drop_project_default_router_mode` applied with `--remote` on preview and production before deploying the API.
...
- [ ] As a Pro account with an Anthropic key, pick Ultra and confirm the reply comes from Claude Fable 5.1 with high reasoning; remove the key and confirm the same tier falls back to the OpenRouter or Workers AI entry.
...
- [ ] Open `/models` signed out and signed in; confirm each tier card shows the headline model and, when signed in, an "On your plan" line where the account resolves differently.
