...
- [ ] During a run, send an instruction from the composer and confirm the Activity timeline shows it as submitted, then received after the worker picks it up. Reload and confirm the entries remain and the composer stays in steering mode until the run ends.
