...
- [x] Leave an interaction unresolved beyond its seven-day recovery window. Confirm it displays expired, cannot authorise work and offers refresh.
- [x] Force task dispatch to fail after the answer or approval is saved. Confirm iPhone reports that the response was received but provider continuation was interrupted, without presenting an approval as rejected.
...
- [x] Inject an unknown interaction type and a protocol version newer than the client. Confirm the card is visible but non-actionable, directs the person to update or refresh, and the rest of the conversation remains usable.
...
- The previously passing native utility suite refuses unknown kinds and protocol v2. ProjectTaskInteractionCard renders a non-actionable update/refresh state and an expired refresh state. Passing API interaction-recovery tests expire the exact seven-day boundary; these are simulated/native-controller and source checks, not physical-device certification.

## Additional automated evidence — 9 September 2026

- The passing native controller test `conversationManagerRecognisesAnApprovalSavedBeforeProviderInterruption` receives a 503 after an approved decision is saved, refreshes the authoritative interrupted interaction and retains `approved` with acknowledged submission. This validates the dispatch-failure boundary without a physical-device claim.
