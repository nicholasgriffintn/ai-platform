...
- [x] Trigger a slash command or teammate mention so a token is inserted into a long draft, and confirm the caret stays visible and the token renders in place.
...
- All eight composer size and draft journeys passed in `test-results/container/f7e4e1a8/results.json`. The 400-line draft retains every line after inserting a real skill token, keeps the token and caret visible, and accepts further typing. Five DOM regression cases also cover line breaks, empty lines and cursor offsets.
