...
- **Change:** Install a per-user Polychat background service from the same desktop binary on macOS and Linux, with keychain authentication, a single-instance lock, and periodic machine heartbeats.
...
- [ ] Attempt to start a second service instance; confirm the data-directory lock refuses it without corrupting the local store.
- [ ] Sign out or remove the session key before installing/running the service; confirm it refuses to start and does not send a heartbeat.
