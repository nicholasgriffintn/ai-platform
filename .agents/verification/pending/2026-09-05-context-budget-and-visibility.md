...
- [x] Compare a provider that reports input tokens with one that does not. Confirm the view says **reported** only for actual telemetry and **estimated** otherwise.
...
- [x] Resume a waiting run and confirm the old attempt's context disappears until the successor records its own step.
...
- All 2,082 API tests passed across 288 files (`/tmp/polychat-api-verification-batch.log`). context-budget.test.ts verifies actual telemetry is reported and missing telemetry estimated. ContextDetailsPanel renders that source; native ChatContextPresentation uses the same distinction and its previously passing native test covers estimated usage.

## Automated evidence — 9 September 2026

- Reviewed the atomic resume update: incrementing the attempt clears both context_json and retry_json. The passing repository suite verifies expected waiting-attempt ownership and exact active-attempt context writes, so an old writer cannot restore its snapshot after resume. Both consumers read the current run context rather than carrying the previous attempt forward. This is repository/consumer-boundary evidence.
