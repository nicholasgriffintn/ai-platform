...
- [ ] Start a multi-step stored task on web, disconnect after an intermediate tool or assistant message, and confirm recovery keeps showing the same run before reconciling its complete final message set.
- [ ] Repeat with the connection lost before the run receipt arrives and confirm the submitted command resolves to the accepted run without starting another task.
...
**Expected interval:** active clients refresh at most every two seconds. A live owner checks authoritative cancellation at roughly one-second intervals between safe model/tool boundaries; an already-running provider or external tool call may extend final interruption.
