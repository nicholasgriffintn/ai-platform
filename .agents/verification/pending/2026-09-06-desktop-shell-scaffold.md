...
- [ ] Press Collect under Diagnostics and confirm it reports the version, platform, database path and keychain reachability, and that it contains no session token or secret.
- [ ] On Linux or Windows, build the application and run `POLYCHAT_DESKTOP_BINARY=<path> pnpm --filter @assistant/desktop test:e2e`; confirm the window opens and every case passes, including the one asserting the window cannot reach a runtime directly.
