...
- [x] Use a provider response without usage telemetry. Confirm recorded consumption says **unknown**, never zero, while any context token estimate remains labelled estimated.
...
- [x] In a safe test environment, age a held chat-run reservation past 24 hours and run scheduled maintenance. Confirm it becomes released and the account reserved balance falls once even if maintenance repeats.
...
- The passing usage service test returns unknown measurement and null consumption when provider events are absent. ContextDetailsPanel and native ChatContextView render unknown consumption separately from estimated context tokens.

## Automated evidence — 9 September 2026

- New real-D1 `UsageReservationBalance.test.ts` passes: an expired held reservation releases its balance once, duplicate settlement returns null, and a replacement hold survives a stale release using the old reservation ID. Maintenance now passes the exact ID from its expired-record scan, preventing a delayed scan from releasing a successor. The maintenance boundary test passes alongside the database regression; this uses an injected date rather than waiting a day.
