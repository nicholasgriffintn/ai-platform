...
function createInternalRequest(headers: Record<string, string> = {}): Request {
	return new Request("https://training.worker.internal/jobs", { headers });
}
...
describe("internal worker auth", () => {
	...
	it("rejects requests without the shared token", () => {
		expect(() =>
			assertInternalRequest(createInternalRequest(), { TRAINING_WORKER_TOKEN: token }),
	...
	it("accepts requests with the shared token and internal user context", () => {
		const request = createInternalRequest({
			...
			[TRAINING_WORKER_USER_ID_HEADER]: "42",
		});

		assertInternalRequest(request, { TRAINING_WORKER_TOKEN: token });
	...
	it("rejects invalid internal user context", () => {
		const request = createInternalRequest({
			[TRAINING_WORKER_TOKEN_HEADER]: token,
			[TRAINING_WORKER_USER_ID_HEADER]: "not-a-user-id",
		});
