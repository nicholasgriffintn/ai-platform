...
function scoreItem(item: CapabilityDiscoveryItem, query: string): number {
	...
	if (name === normalisedQuery || id === normalisedQuery) score += 100;
	if (name.includes(normalisedQuery) || id.includes(normalisedQuery)) score += 30;
	if (description.includes(normalisedQuery)) score += 15;

	for (const term of terms) {
		if (name.includes(term) || id.includes(term)) score += 10;
