...
describe("normaliseTeammateResponse", () => {
  ...
  it("coerces the stored temperature text to a number", () => {
    expect(normaliseTeammateResponse(buildStoredTeammate({ temperature: "0.7" })).temperature).toBe(
      0.7,
    );
    expect(
      normaliseTeammateResponse(buildStoredTeammate({ temperature: "warm" })).temperature,
    ).toBeNull();
    expect(
      normaliseTeammateResponse(buildStoredTeammate({ temperature: null })).temperature,
    ).toBeNull();
  });
