...
export interface MCPToolName {
  shortTeammateId: string;
  toolName: string;
}

export interface TeammateMCPToolDefinition {
  name: string;
  description?: string;
...
const MCP_TEAMMATE_KEY_LENGTH = 8;

export function toMCPTeammateKey(teammateId: string): string {
  return teammateId.substring(0, MCP_TEAMMATE_KEY_LENGTH);
}

export function createMCPToolName(teammateId: string, toolName: string): string {
  return `mcp_${toMCPTeammateKey(teammateId)}_${toolName}`;
}
...
export function resolveMCPAIToolDefinition(
  ...
  name: string,
  definition: unknown,
): TeammateMCPToolDefinition | null {
  ...
  return {
    name: createMCPToolName(teammateId, name),
