...
function schemaNodeAcceptsFiles(value: unknown, visited: Set<object>): boolean {
  ...
  if (isRecord(value.properties)) {
    for (const [name, property] of Object.entries(value.properties)) {
      if (FILE_FIELD_NAME.test(name) || schemaNodeAcceptsFiles(property, visited)) {
        return true;
      }
    }
  }
  ...
  for (const key of ["items", "anyOf", "allOf", "oneOf", "$defs", "definitions"]) {
    ...
    if (Array.isArray(child)) {
      if (child.some((item) => schemaNodeAcceptsFiles(item, visited))) {
        return true;
      }
    } else if (isRecord(child)) {
      if (Object.values(child).some((item) => schemaNodeAcceptsFiles(item, visited))) {
        return true;
      }
    }
  }
