...
export async function getPersonalSkill(
	...
	if (!skill) throw new AssistantError("Skill not found", ErrorType.NOT_FOUND, 404);
	return skill;
}
...
export async function updatePersonalSkill(
	...
	if (!existing) throw new AssistantError("Skill not found", ErrorType.NOT_FOUND, 404);
	const document = parseAuthoredSkillDocument(input.content);
...
export async function getProjectSkill(
	...
	if (!capability) throw new AssistantError("Skill not found", ErrorType.NOT_FOUND, 404);
