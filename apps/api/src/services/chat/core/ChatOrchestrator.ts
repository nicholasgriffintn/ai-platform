...
import { runAgentLoop } from "~/services/chat/agent/agent-loop";
import { createGoalFinishGate } from "~/services/chat/agent/goal-gate";
import {
  createBufferedTurnTransport,
  createStreamingTurnTransport,
} from "~/services/chat/agent/turn-transport";
import { createChatRunReceiptStream, createChatTurnStream } from "~/services/chat/core/chat-stream";
import { prependCompactionStateEvent } from "~/services/chat/core/compaction-stream";
import { createChatExecutionRequest } from "~/services/chat/core/execution-request";
import { createModelEnsembleStream } from "~/services/chat/core/model-ensemble";
import { buildToolRequestContext } from "~/services/chat/core/request-context";
import { resolveRunProvenance } from "~/services/chat/core/run-provenance";
import { resolveAccountDefaultComputeSite } from "~/services/chat/policy/project-model-tier";
import { resolveTurnStepBudget } from "~/services/chat/policy/step-budget";
import { applyTierReasoningEffort } from "~/services/chat/policy/tier-reasoning";
import { RequestPreparer, type PreparedRequest } from "~/services/chat/preparation/RequestPreparer";
import { watchTurnCancellation } from "~/services/chat/streaming/turn-cancellation";
import { ValidationPipeline } from "~/services/chat/validation/ValidationPipeline";
import {
  acquireThread,
  threadLockError,
  type ThreadLease,
...
const logger = getLogger({ prefix: "services/chat/core/ChatOrchestrator" });

const THREAD_LOCK_RETRY_INTERVAL_MS = 200;
const THREAD_LOCK_RETRY_TIMEOUT_MS = 10_000;
...
export class ChatOrchestrator {
  ...
  private async holdThreadForTurn(options: CoreChatOptions): Promise<ThreadLease | undefined> {
    ...
    const deadline = Date.now() + THREAD_LOCK_RETRY_TIMEOUT_MS;
    ...
    for (;;) {
      ...
      if (lock.reason !== "busy" || Date.now() >= deadline) {
        throw threadLockError(lock);
      }

      await sleep(THREAD_LOCK_RETRY_INTERVAL_MS);
    }
  }
  ...
  private async executeRequest(
    ...
    try {
      ...
      try {
        await runResult.postTurn?.();
      } catch (error) {
        logger.error("Failed to finish post-turn work", {
          error,
          completionId: chatOptions.completion_id,
        });
      }
