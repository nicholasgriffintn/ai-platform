...
import { createChatTurnStream } from "~/services/chat/core/chat-stream";
...
describe("createChatTurnStream", () => {
  ...
  it("closes the visible stream before post-turn work finishes", async () => {
    let completePostTurn: () => void = () => {};

    const postTurn = new Promise<void>((resolve) => {
      completePostTurn = resolve;
    });

    mocks.runAgentLoop.mockResolvedValue({
      response: {},
      postTurn: vi.fn(() => postTurn),
    });
