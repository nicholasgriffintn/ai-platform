...
import { createChatSseStreamWriter } from "~/services/chat/streaming/emitter";

describe("turn continuity telemetry", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.createMetrics.mockReturnValue({ recordMetric: mocks.recordMetric });
  });
  ...
  it("cannot affect a turn when monitoring fails", () => {
    mocks.createMetrics.mockImplementation(() => {
