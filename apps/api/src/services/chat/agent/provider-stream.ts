import {
  ...
  modelEmitsUnterminatedThinking,
  ResponseFormatter,
  StreamingFormatter,
...
} from "~/services/chat/messages/anthropic-response-parts";
import { GoogleCodeExecutionCollector } from "~/services/chat/messages/google-response-parts";
import {
  appendReasoningPart,
  appendTextPart,
  buildMessageParts,
} from "~/services/chat/messages/parts";
import type { ChatEventSink } from "~/services/chat/streaming/emitter";
import { SseLineBuffer } from "~/services/chat/streaming/sse-line-buffer";
import { writeTurnActivity } from "~/services/chat/streaming/turn-activity";
import {
  emitToolInputDelta,
  emitToolInputStart,
  emitToolInputStop,
} from "~/services/chat/tools/events";
...
const logger = getLogger({ prefix: "services/chat/agent/provider-stream" });
...
export async function consumeProviderStream(
  ...
  const appendCompletedResponseAssets = async (data: unknown) => {
    ...
    try {
      formattedResponse = await ResponseFormatter.formatResponse(data.response, provider, {
        ...
        storage: providerHost.storage.forContext(context.serviceContext ?? { env }),
        completion_id: completionId,
        is_streaming: false,
  ...
  const handleLine = async (line: string): Promise<boolean> => {
    ...
    const formattedData = await ResponseFormatter.formatResponse(data, provider, {
      model,
      modalities: modelConfig?.modalities,
      storage: providerHost.storage.forEnv(env),
      is_streaming: true,
      userId: context.userId,
    });
