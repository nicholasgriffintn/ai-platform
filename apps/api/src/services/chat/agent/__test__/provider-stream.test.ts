...
vi.mock("~/lib/providers/host", () => {
  let stored = 0;
  const storage = {
    storeOutputFile: vi.fn(async () => {
      const index = stored++;

      return {
        outputId: `output-${index}`,
        key: `${index}.png`,
        url: `https://assets.test/${index}.png`,
      };
    }),
  };
  ...
  return {
    providerHost: {
      storage: { forEnv: () => storage, forContext: () => storage },
    },
  };
});

import { consumeProviderStream } from "~/services/chat/agent/provider-stream";
import { createStreamingTurnTransport } from "~/services/chat/agent/turn-transport";
