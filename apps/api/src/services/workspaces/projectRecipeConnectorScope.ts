...
export function resolveProjectRecipeConnectorScope(
	...
	for (const recipe of assistantRecipes) {
		...
		for (const provider of buildAllowedConnectorProviders(recipe)) providers.add(provider);
		for (const [provider, operations] of Object.entries(buildAllowedConnectorOperations(recipe))) {
			const scopedOperations =
				operationsByProvider.get(provider as RecipeConnectorProvider) ?? new Set();
			for (const operation of operations) scopedOperations.add(operation);
			operationsByProvider.set(provider as RecipeConnectorProvider, scopedOperations);
		}
	}
