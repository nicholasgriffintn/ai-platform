...
export async function sendTeammateModerationNotification(
  ...
  const subject = isApproved
    ? `🎉 Your agent "${teammateName}" has been approved for the marketplace`
    : `📝 Your agent "${teammateName}" needs attention`;
  ...
  const template = createEmailTemplate({
    ...
    content: `
      <p><strong>Your agent "${teammateName}" has been reviewed and ${status} for the ${APP_NAME} marketplace.</strong></p>
  ...
  try {
    ...
    logger.info(`Sent moderation notification to ${userEmail} for agent ${teammateName}`);
  } catch (error) {
...
export async function sendTeammateFeaturedNotification(
  ...
  const subject = `🌟 Your agent "${teammateName}" has been featured!`;

  const template = createEmailTemplate({
    subject,
    title: "🌟 Your Agent has been Featured!",
    content: `
      <p><strong>🎉 Congratulations! Your agent "${teammateName}" has been selected as a featured agent on the ${APP_NAME} marketplace.</strong></p>
  ...
  try {
    await sendEmail(env, userEmail, template.subject, template.bodyText, template.bodyHtml);
    logger.info(`Sent featured notification to ${userEmail} for agent ${teammateName}`);
  } catch (error) {
