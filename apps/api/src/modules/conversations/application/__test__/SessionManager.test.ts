...
} from "~/modules/conversations/application/SessionManager";
...
describe("SessionManager", () => {
  ...
  beforeEach(() => {
    ...
    mockGenerateText.mockResolvedValue("Summarised archived messages");
  });
  ...
  it("persists and archives exactly the messages represented within the summary input cap", async () => {
    ...
    const summaryRequest = mockGenerateText.mock.calls[0]?.[0];
    ...
    expect(summaryRequest.prompt).not.toContain("retain audit records");
  });
