...
vi.mock("~/modules/chat/application/validation/ValidationPipeline", () => ({
...
vi.mock("~/modules/chat/application/preparation/RequestPreparer", () => ({
...
vi.mock("~/modules/chat-runs/application/lifecycle", () => ({
  acceptChatRun: mockAcceptChatRun,
  findAcceptedChatRunCommand: mockFindAcceptedChatRunCommand,
}));

vi.mock("~/modules/chat/application/streaming/responses", () => ({
  getAIResponse: mockGetAIResponse,
}));

vi.mock("~/modules/chat/application/agent/provider-stream", () => ({
  consumeProviderStream: mockConsumeProviderStream,
}));

vi.mock("~/modules/chat/application/tools/execution", () => ({
  handleToolCalls: mockHandleToolCalls,
}));

vi.mock("~/modules/conversations/application/SessionManager", () => ({
...
vi.mock("~/infrastructure/providers/capabilities/guardrails", () => ({
  Guardrails: class {
    constructor() {
  ...
  describe("process", () => {
    ...
    describe("successful processing", () => {
      ...
      it("refuses a turn while another operation holds the conversation", async () => {
        mockAcquireThread.mockResolvedValue({
          acquired: false,
        ...
        try {
          const pending = orchestrator.process(mockOptions);
      ...
      it("rechecks command acceptance when a concurrent original wins the lease", async () => {
        ...
        let result: Awaited<ReturnType<typeof orchestrator.process>>;
