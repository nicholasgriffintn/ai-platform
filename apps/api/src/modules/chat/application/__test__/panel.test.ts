import { ResponseFormatter, resolveEffectiveMaxTokens } from "@ngriffin_uk/polychat-ai-providers";
import { beforeEach, describe, expect, it, vi } from "vitest";

import { extractPanelRouting, runPanel, type PanelMember } from "~/modules/chat/application/panel";
import type { ChatCompletionParameters, IEnv } from "~/types";

const mocks = vi.hoisted(() => ({
  getAIResponse: vi.fn(),
  recordModelTurnUsage: vi.fn(
    async (_runtime: unknown, _params: { messageId?: string }) => undefined,
  ),
}));
...
vi.mock("~/infrastructure/context/serviceContext", () => ({ createServiceContext: () => ({}) }));
