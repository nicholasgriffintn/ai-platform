...
vi.mock("~/modules/models/application/resolve", async (importOriginal) => {
  const actual = await importOriginal<typeof import("~/modules/models/application/resolve")>();
...
import { resolveSandboxApiBaseUrl } from "~/modules/apps/application/sandbox/urls";

import { resolveSandboxModel } from "../worker";

describe("resolveSandboxApiBaseUrl", () => {
  it("uses API_BASE_URL", () => {
    const env = {
      API_BASE_URL: "http://localhost:8787",
    } as any;

    expect(resolveSandboxApiBaseUrl(env)).toBe("http://localhost:8787");
  });

  it("falls back to production API URL when API_BASE_URL is missing", () => {
    const env = {} as any;

    expect(resolveSandboxApiBaseUrl(env)).toBe("https://api.polychat.app");
  });
});
