...
export function getPrivateFileAccessScope(
	...
	if (record.project_id) return userId ? "project" : "denied";
	if (userId === record.created_by_user_id) return "owner";
	if (record.conversation_id) return "public-conversation";
	return "denied";
}
...
async function assertFileAccess(
	...
	if (scope === "public-conversation") {
		...
		if (conversation && isPublicConversation(conversation.is_public)) return;
	}
...
export async function readPrivateFile(params: {
	...
	if (!object) throw new AssistantError("File not found", ErrorType.NOT_FOUND, 404);
