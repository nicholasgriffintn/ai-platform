...
export function isComposioProviderConfigured(
	...
	if (provider.auth.authType !== "composio" || !isComposioConfigured(env)) return false;
	return true;
}
...
function getErrorType(status: number): ErrorType {
	if (status === 401 || status === 403) return ErrorType.AUTHORISATION_ERROR;
...
function parseConnectedAccount(value: unknown): ComposioConnectedAccount | null {
	if (!isRecord(value) || !isRecord(value.toolkit)) return null;
	if (
...
function parseToolSchemaIdentity(
	...
	if (provider.auth.authType !== "composio") return null;
	if (
		!isRecord(value) ||
...
export async function createComposioToolSession(params: {
	...
	const availableTools = new Set(
		params.provider.operations
			.filter((operation) =>
				operation.authConfigIds?.includes(params.connectedAccount.authConfigId as string),
			)
...
export async function disconnectComposioAccount(params: {
	...
	if (params.revokeAtProvider !== false) {
		try {
			await composioRequest<unknown>({
				env: params.env,
	...
	try {
		...
		if (!(error instanceof AssistantError) || error.statusCode !== 404) throw error;
	}
}
