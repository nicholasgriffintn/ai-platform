...
import { getSageMakerTrainingDeploymentRuntimeRecordByEndpointName } from "~/modules/models/application/training-deployments";
import type { TrainingDeploymentRuntimeRecord } from "~/modules/models/application/training-deployments";
import type { ChatCompletionParameters } from "~/types";

import { getTrainingModel } from "./modelCatalog";
...
function getSageMakerRuntimeErrorMessage(data: unknown, fallback: string): string {
  if (!isRecord(data)) {
    return fallback || "SageMaker runtime request failed";
  }
