import { SANDBOX_PROMPT_STRATEGIES, type SandboxPromptStrategy } from "@assistant/schemas";
...
const PROMPT_STRATEGY_DESCRIPTIONS: Record<SandboxPromptStrategy, string> = {
	...
	refactor: "Prioritise maintainability improvements while preserving behaviour.",
	"test-hardening": "Focus on high-signal validation, state, and error-path coverage.",
};

export const sandboxPromptStrategyOptions: PromptStrategyOption[] = SANDBOX_PROMPT_STRATEGIES.map(
	(strategy) => ({
		value: strategy,
...
export function getSandboxPromptStrategyLabel(strategy?: SandboxPromptStrategy): string {
	if (!strategy) {
...
export function getSandboxPromptStrategyDescription(strategy: SandboxPromptStrategy): string {
	return PROMPT_STRATEGY_DESCRIPTIONS[strategy];
}

export function isSandboxPromptStrategy(value: string): value is SandboxPromptStrategy {
