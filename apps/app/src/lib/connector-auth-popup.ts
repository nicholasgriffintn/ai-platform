...
export function waitForConnectorAuthPopup(params: {
	...
	return new Promise((resolve) => {
		...
		const finish = (outcome: ConnectorAuthPopupOutcome) => {
			...
			if (outcome === "connected" && !params.popup.closed) params.popup.close();
			resolve(outcome);
		};
		const onMessage = (event: MessageEvent) => {
			if (readCompletionMessage(event, params.popup, params.provider)) finish("connected");
		};
		...
		const closedInterval = window.setInterval(() => {
			...
			if (!params.popup.closed || closedTimer !== undefined) return;
