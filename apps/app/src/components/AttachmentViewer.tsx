          ...
          <div key={att.url} className="p-3 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-muted-foreground flex gap-2">
                ...
                {att.size ? (
                  <span>
                    {Math.round((att.size / 1024 / 1024) * 10) / 10} MB
                  </span>
                ) : null}
              </div>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-xs inline-flex items-center gap-1 px-2 py-1 border rounded"
            >
...
function Preview({ attachment }: { attachment: Attachment }) {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="block h-28 w-full bg-zinc-100 dark:bg-zinc-800 rounded flex items-center justify-center text-xs"
    >
      {attachment.type === "document" || attachment.type === "markdown_document"
        ? "Document"
