...
import { AUTH_DAILY_MESSAGE_LIMIT, DAILY_LIMIT_PRO_MODELS } from "~/lib/usage-limits";
...
function describePlanAllowance(plan: Plan): string[] {
  if (typeof plan.included_credits === "number" && plan.included_credits > 0) {
    const lines = [`${formatCredits(plan.included_credits)} credits every month`];
  ...
  return [
    `${AUTH_DAILY_MESSAGE_LIMIT} standard messages a day`,
    `${DAILY_LIMIT_PRO_MODELS} premium messages a day`,
    "unlimited messages on your own provider keys",
  ];
}
