...
export default function AttachmentUploader({ value = [], onChange, multiple = true }: AttachmentUploaderProps) {
  ...
  const [{ files, isDragging, errors }, { removeFile, openFileDialog, getInputProps, handleDrop, handleDragEnter, handleDragLeave }] =
    useFileUpload({ accept, multiple, maxFiles: multiple ? 10 : 1, onFilesAdded: async (added) => void uploadQueued(added) });
  ...
  async function uploadQueued(added: FileWithPreview[]) {
    for (const f of added) {
      ...
      try {
        const res = await fetchApi("/uploads", { method: "POST", body: form, headers });
        if (!res.ok) {
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Images, PDFs, audio, and video</p>
        </div>
      ...
      <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-3">
        {attachments.map((att, idx) => (
          <div key={`${att.url}-${idx}`} className="border rounded p-2 text-xs">
            <div className="truncate font-medium" title={att.name || att.url}>{att.name || att.url}</div>
            <div className="flex items-center justify-between mt-1">
              <span className="px-1 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800">{att.type}</span>
              {att.size ? <span>{formatBytes(att.size)}</span> : null}
            </div>
            {att.metadata?.thumbnailUrl && (
              <img src={att.metadata.thumbnailUrl} alt={att.name || "thumbnail"} className="mt-2 h-24 w-full object-cover rounded" />
            )}
          </div>
        ))}
        ...
        {files.map((f) => (
          <div key={f.id} className="border rounded p-2 text-xs">
            <div className="truncate font-medium" title={(f.file as any).name || f.id}>{(f.file as any).name || f.id}</div>
            {f.preview && (f.file as File)?.type?.startsWith("image/") && (
              <img src={f.preview} alt="preview" className="mt-2 h-24 w-full object-cover rounded" />
            )}
