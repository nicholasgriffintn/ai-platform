...
export function useConnectorSetup({
	...
	const startComposioConnector = async (
		...
		try {
			...
			if (popupAbortRef.current === abortController) popupAbortRef.current = null;
	...
	return {
		...
		closeAuthConfigDialog: (open) => {
			if (!open) setAuthConfigDialog({ connector: null, configs: [] });
		},
		connect,
		...
		onApiKeyStored: async () => {
			...
			if (provider) await refreshConnection(provider);
		},
		selectAuthConfig,
	};
}
