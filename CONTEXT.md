...
- **Assistant capability**: a product-level capability shown through app, recipe, connector, agent, or tool catalogues. Capabilities publish availability, auth state, read/write risk, approval policy, required model capabilities, required connector state, launch method, execution mode, saved-state support, availability reason, and tags while keeping their runtime-specific implementation separate.
...
- **Assistant action catalogue seam**: `packages/schemas/src/assistant-actions.ts` owns the shared discovery, capability, and launch contract for assistant actions across installed recipes, dynamic apps, connectors, agents, and model tools. Frontend composer modules should consume catalogue items and execute their `launch` contract instead of branching on display kind or rebuilding URL/tool/recipe intent locally.
